...
parser.add_argument('--nb_samples', dest='nb_samples', type=int, default=10000, help="Choose the size of the training dataset")
...
def main():
    ...
    if args.phase == 'train':
        training(x_train=x_train, y_train=y_train, model=args.model, nb_epoch=args.epoch, training_batch_size=args.batch_size, file=filename)

    elif args.phase == 'test':
        testing(x_test=x_test, y_test=y_test, model=args.model, file=filename)
